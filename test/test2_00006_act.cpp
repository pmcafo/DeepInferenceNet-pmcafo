...
public:
    ...
    {
        Tensor* y = act->create_tensors(x);
        return y;
    }

    virtual Tensor* feed_forward(Tensor* x)
    {
        Tensor* y = act->feed_forward(x);
        return y;
    }
private:
};
...
{
/*
python build.py --platform LINUX --cxx g++ --backend BACKEND_X86 --exec_file test2_00006_act
