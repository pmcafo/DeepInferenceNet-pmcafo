...
#if BACKEND_X86
#include "common/avgpool2d_common.h"
#endif // BACKEND_X86
...
AvgPool2d::AvgPool2d(int kernel_size, int stride, int padding, bool ceil_mode)
{
...
AvgPool2d::AvgPool2d(int kernel_h, int kernel_w, int stride_h, int stride_w, int padding_h, int padding_w, bool ceil_mode)
{
...
Tensor* AvgPool2d::create_tensors(Tensor* input)
{
...
Tensor* AvgPool2d::feed_forward(Tensor* input)
{
    Tensor* output = output_tensors->at(0);
    miemienet::functional::avgpool2d(input, output, kernel_h, kernel_w, stride_h, stride_w, padding_h, padding_w, ceil_mode);
    return output;
}
